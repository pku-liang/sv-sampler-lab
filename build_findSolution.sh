...
LINK_FLAGS="-L$CUDD_LIB -lcudd -lm -lquadmath"  # 添加 quadmath 库支持 __float128 类型
...
if [ $? -eq 0 ]; then
    ...
    echo "使用方法: $RUN_DIR/$EXEC_NAME <aag_file>  <random_seed> <solution_num> <output_file>"
    ...
    if [ $# -ge 4 ]; then
        ...
        echo "提示：您可以通过运行 '$0 <aag_file> <random_seed> <solution_num> <output_file> ' 来直接编译并执行程序"
    fi
else
